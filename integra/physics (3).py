...
rastvor = [wood, sand, iron, stone, grass, ground]
spc = [wood, grass]
...
class Dust(Substance):  # песок, зола
    ...
    def action(self):
        ...
        # ИСПРАВЛЕНО: правильные координаты для проверки ВНИЗ
        ...
        if down_color in phantom:
            if down:
                down.move_to(x, y)  # перемещаем то, что под нами, на наше место
            self.move_to(x, y - 1)  # падаем вниз
